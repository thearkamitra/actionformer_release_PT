...
def load_pred_seg_from_json(json_file, label='label', label_offset=0):
    # load json file
    ...
    for k, v, in json_db.items():
        ...
        # for each event
        for event in v:
            starts += [float(event['timestamps'][0])]
            stops += [float(event['timestamps'][1])]
            if isinstance(event[label], (Tuple, List)):
                # offset the labels by label_offset
                label_id = 0
                for i, x in enumerate(event[label][::-1]):
...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if not os.path.isfile(preds):
        	raise FileNotFoundError(f'file {preds} does not exist.')

        preds = load_pred_seg_from_json(preds)

        if self.ground_truth['video-id'].nunique() != preds['video-id'].nunique():
        	raise ValueError(f"Missing Video IDs in submission. Expected {self.ground_truth['video-id'].nunique()}, got {preds['video-id'].nunique()}")
        ...
        if verbose:
            ...
            for tiou, tiou_mAP in zip(self.tiou_thresholds, mAP):
                ...
                for idx, k in enumerate(self.top_k):
                    block += 'Recall@{:d}x = {:>4.2f} (%) '.format(k)
            print(block)
...
def evaluate(test_annotation_file, user_submission_file, phase_codename, **kwargs):
	...
	import time
	tic = time.time()
	det_eval = ANETdetection(test_annotation_file, phase_codename,tiou_thresholds = np.linspace(0.1, 0.5, 5))
	_, mAP = det_eval.evaluate(user_submission_file, verbose=True)
	toc = time.time()
	print(f'time: {toc-tic}')
	output = {}
	if phase_codename == "valid":
		output["result"] = [
			{
				"valid_split": {
				  "mAP": mAP
		...
		output["submission_result"] = output["result"][0]["valid_split"]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
      description='Evaluate model outputs on Perception Test challenge data')
    parser.add_argument('labels', type=str, metavar='DIR',
                        help='Path to ground truth annotations')
    parser.add_argument('results', type=str, metavar='DIR',
                        help='Path to model outputs')
    args = parser.parse_args()
    phase = 'valid'             
    output = evaluate(args.labels, args.results, phase)
